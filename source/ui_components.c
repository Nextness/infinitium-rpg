           ...
           rpg_game_state_t *gs inout(), callback_t on_click in(), callback_t on_change in())
{
    ...
    if (!rl_check_collision_point_rec(mouse_position, button.button_area)) {
        btn_color_hsv.z = on_change(gs) ? 0.8f : 0.5f;
        border_color_hsv.z = on_change(gs) ? 0.8f : 0.5f;
        goto render_button_l;
    }

    // Hover button state
    if (!rl_is_mouse_button_released(MOUSE_BUTTON_LEFT)) {
        btn_color_hsv.z = on_change(gs) ? 0.9f : 0.6f;
        border_color_hsv.z = on_change(gs) ? 0.9f : 0.6f;
        goto render_button_l;
    }
